...
for ip in array:
    ...
    try:
        ...
        onlyipstr = onlyip[0]
        print(ip)
        resource='/provisioning/symmetrix'
        url = ip+resource
                ###pdb.set_trace()
        with no_ssl_verification():
            ...
            for Symmetrix in Symmetrixname:
                ...
                directoru = '/provisioning/symmetrix/'+Symmetrix+'/director/'
                ...
                if(Dir_count > 0 ):
                    ...
                    if(len(FA_Dirs) > 0):
                        ...
                        for FA_Dir in FA_Dirs:
                            ...
                            portu = '/provisioning/symmetrix/'+Symmetrix+'/director/'+FA_Dir
                            ...
                            ##https://10.254.64.174:8443/univmax/restapi/provisioning/symmetrix/000192601394/director/FA-6F/port###

                            if(FA_ports > 0):

                                #pdb.set_trace()

                                eachport = '/provisioning/symmetrix/'+Symmetrix+'/director/'+FA_Dir+'/port'
                                ...
                                for finalport in Port_id:
                                    ...
                                    finalur = '/provisioning/symmetrix/'+Symmetrix+'/director/'+FA_Dir+'/port/'+finalport
                                    ...
                                    final_data = [[onlyipstr,db_Symmetrix,db_FA_dir,db_Identifier_id,db_finalport,today_date]]
                                    ...
                                    mycursor.executemany("""INSERT INTO zoningReport(Hostname,Array,FaPort,Identifier,FaPortg,todaydate) VALUES(%s,%s,%s,%s,%s,%s)""",final_data)
                                    conn.commit()
                                    mycursor.close()
